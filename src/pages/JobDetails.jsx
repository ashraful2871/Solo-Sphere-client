...
const JobDetails = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (compareAsc(new Date(), new Date(deadline)) === 1) {
      return toast.error("Deadline over biding forbidden");
    }

    // 3. price with in maximum price validation
    if (price > max_price) {
      return toast.error("Offer less or at least equal to maximum price ");
    }

    //4. offer deadline with in sellers deadline validation
    if (compareAsc(new Date(startDate), new Date(deadline)) === 1) {
      return toast.error("Please offer a date within deadline ");
    }
